...
class ADS1x15:
    ...
    __ADS1015_REG_CONFIG_CQUE_1CONV   = 0x0000  # Assert ALERT/RDY after one conversion
    ...
    def __init__(self, address=0x48, ic=__IC_ADS1015, debug=False):
        ...
        if (ic < self.__IC_ADS1015) or (ic > self.__IC_ADS1115):
            if self.debug:
                print("ADS1x15: Invalid IC specified: %d" % ic)
            return -1
        else:
            self.ic = ic
    ...
    def readADCSingleEnded(self, channel=0, pga=6144, sps=250):
        "Gets a single-ended ADC reading from the specified channel in mV. The sample rate for this mode (single-shot) can be used to lower the noise (low sps) or to lower the power consumption (high sps) by duty cycling, see datasheet page 14 for more info. The pga must be given in mV, see page 13 for the supported values."
        ...
        if self.ic == self.__IC_ADS1015:
            ...
            if (sps not in self.spsADS1115) and self.debug:
        ...
        if (pga not in self.pgaADS1x15) and self.debug:
        ...
        bytes_to_write = [(config >> 8) & 0xFF, config & 0xFF]
        self.i2c.writeList(self.__ADS1015_REG_POINTER_CONFIG, bytes_to_write)
    ...
    def readADCDifferential(self, chP=0, chN=1, pga=6144, sps=250):
        "Gets a differential ADC reading from channels chP and chN in mV. The sample rate for this mode (single-shot) can be used to lower the noise (low sps) or to lower the power consumption (high sps) by duty cycling, see data sheet page 14 for more info. The pga must be given in mV, see page 13 for the supported values."
        ...
        # Set samples per second, defaults to 250sps.
        # If sps is in the dictionary it returns the corresponding constant,
        # otherwise it returns the value for 250sps.
        if self.ic == self.__IC_ADS1015:
            config |= self.spsADS1015.setdefault(sps, self.__ADS1015_REG_CONFIG_DR_1600SPS)
        else:
            if (sps not in self.spsADS1115) and self.debug:
        ...
        self.i2c.writeList(self.__ADS1015_REG_POINTER_CONFIG, config_bytes)
    ...
    def readADCDifferential01(self, pga=6144, sps=250):
        "Gets a differential ADC reading from channels 0 and 1 in mV. The sample rate for this mode (single-shot) can be used to lower the noise (low sps) or to lower the power consumption (high sps) by duty cycling, see data sheet page 14 for more info. The pga must be given in mV, see page 13 for the supported values."
        return self.readADCDifferential(0, 1, pga, sps)
    
    
    def readADCDifferential03(self, pga=6144, sps=250):
        "Gets a differential ADC reading from channels 0 and 3 in mV. The sample rate for this mode (single-shot) can be used to lower the noise (low sps) or to lower the power consumption (high sps) by duty cycling, see data sheet page 14 for more info. The pga must be given in mV, see page 13 for the supported values."
        return self.readADCDifferential(0, 3, pga, sps)
        
        
    def readADCDifferential13(self, pga=6144, sps=250):
        "Gets a differential ADC reading from channels 1 and 3 in mV. The sample rate for this mode (single-shot) can be used to lower the noise (low sps) or to lower the power consumption (high sps) by duty cycling, see data sheet page 14 for more info must be given in mV, see page 13 for the supported values."
        return self.readADCDifferential(1, 3, pga, sps)   # Note: Fixed function call from __readADCDifferential to readADCDifferential


    def readADCDifferential23(self, pga=6144, sps=250):
        "Gets a differential ADC reading from channels 2 and 3 in mV. The sample rate for this mode (single-shot) can be used to lower the noise (low sps) or to lower the power consumption (high sps) by duty cycling, see data sheet page 14 for more info. The pga must be given in mV, see page 13 for the supported values."
        return self.readADCDifferential(2, 3, pga, sps)
    
    
    def startContinuousConversion(self, channel=0, pga=6144, sps=250): 
        "Starts the continuous conversion mode and returns the first ADC reading in mV from the specified channel. The sps controls the sample rate. The pga must be given in mV, see datasheet page 13 for the supported values. Use getLastConversionResults() to read the next values and stopContinuousConversion() to stop converting."
        ...
        # Write config register to the ADC.
        # Once written, the ADC will convert continuously;
        # subsequent values can be read using getLastConversionResult.
        config_bytes = [(config >> 8) & 0xFF, config & 0xFF]
        self.i2c.writeList(self.__ADS1015_REG_POINTER_CONFIG, config_bytes)
    ...
    def startContinuousDifferentialConversion(self, chP=0, chN=1, pga=6144, sps=250): 
        ...
        # Set samples per second, defaults to 250 sps.
        if self.ic == self.__IC_ADS1015:
            config |= self.spsADS1015.setdefault(sps, self.__ADS1015_REG_CONFIG_DR_1600SPS)
        else:
            if (sps not in self.spsADS1115) and self.debug:	  
        ...
        # Set channels for differential conversion.
        if (chP == 0 and chN == 1):
            config |= self.__ADS1015_REG_CONFIG_MUX_DIFF_0_1
        elif (chP == 0 and chN == 3):
            config |= self.__ADS1015_REG_CONFIG_MUX_DIFF_0_3
        ...
        # Set 'start single-conversion' bit to begin conversions.
        # (This flag remains the same for continuous mode.)
        config |= self.__ADS1015_REG_CONFIG_OS_SINGLE
    
        # Write the config register to the ADC.
        config_bytes = [(config >> 8) & 0xFF, config & 0xFF]
        self.i2c.writeList(self.__ADS1015_REG_POINTER_CONFIG, config_bytes)
    
        # Wait for the ADC conversion to complete.
        # The minimum delay depends on the sps (delay ≥ 1/sps); we add 0.5 ms for safety.
        delay = 1.0 / sps + 0.0005
        time.sleep(delay)
        ...
        result = self.i2c.readList(self.__ADS1015_REG_POINTER_CONVERT, 2)
        if self.ic == self.__IC_ADS1015:
            # For the 12-bit ADS1015, shift right 4 bits and convert to mV.
            return (((result[0] << 8) | (result[1] & 0xFF)) >> 4) * pga / 2048.0
        else:
            # For the ADS1115, take signed values into account.
            val = (result[0] << 8) | result[1]
            if val > 0x7FFF:
                return (val - 0xFFFF) * pga / 32768.0
            else:
    ...
    def stopContinuousConversion(self):
        ...
        # Once written, the ADC will perform one single conversion and then enter power-off mode.
        config = 0x8583  # See page 18 of the datasheet.
        config_bytes = [(config >> 8) & 0xFF, config & 0xFF]
        self.i2c.writeList(self.__ADS1015_REG_POINTER_CONFIG, config_bytes)    
        return True
    ...
    def getLastConversionResults(self):
        ...
        result = self.i2c.readList(self.__ADS1015_REG_POINTER_CONVERT, 2)
        if self.ic == self.__IC_ADS1015:
            # For the ADS1015: shift right 4 bits and convert to mV.
            return (((result[0] << 8) | (result[1] & 0xFF)) >> 4) * self.pga / 2048.0
        else:
            # For the ADS1115, taking signed values into account.
            val = (result[0] << 8) | result[1]
            if val > 0x7FFF:
                return (val - 0xFFFF) * self.pga / 32768.0
            else:
                return val * self.pga / 32768.0
    ...
    def startSingleEndedComparator(self, channel, thresholdHigh, thresholdLow, 
                                ...
                                numReadings=1):
        "Starts the comparator mode on the specified channel, see datasheet pg. 15. In traditional mode the ALERT pin will go low when the voltage exceeds thresholdHigh until it falls below thresholdLow (both given in mV). In window mode (traditionalMode=False) it alerts when the voltage is not between the thresholds. In latching mode the alert will persist until the conversion value is read. numReadings controls how many readings are necessary to trigger an alert (1, 2, or 4). This function starts the continuous conversion mode; use getLastConversionResults() to read the current value (which may differ from the one that triggered the alert) and clear the alert in latching mode. The sps controls the sample rate and the pga the gain (see datasheet page 13)."
        
        # With an invalid channel, return -1.
        if channel > 3:
            if self.debug:
                print("ADS1x15: Invalid channel specified: %d" % channel)
            return -1
        
        # Begin with continuous conversion mode.
        ...
        # Set the number of conversions required to trigger the comparator.
        if numReadings == 4:
        ...
        if self.ic == self.__IC_ADS1015:
            if (sps not in self.spsADS1015) and self.debug:
        ...
        # Write the threshold high register.
        ...
        thresholdHigh_bytes = [(thresholdHighWORD >> 8) & 0xFF, thresholdHighWORD & 0xFF]
        self.i2c.writeList(self.__ADS1015_REG_POINTER_HITHRESH, thresholdHigh_bytes) 
    
        # Write the threshold low register.
        if self.ic == self.__IC_ADS1015:
            thresholdLowWORD = int(thresholdLow * (2048.0 / pga))
        else:
            thresholdLowWORD = int(thresholdLow * (32767.0 / pga))    
        thresholdLow_bytes = [(thresholdLowWORD >> 8) & 0xFF, thresholdLowWORD & 0xFF]
        self.i2c.writeList(self.__ADS1015_REG_POINTER_LOWTHRESH, thresholdLow_bytes)     

        # Write the configuration register to the ADC.
        config_bytes = [(config >> 8) & 0xFF, config & 0xFF]
        self.i2c.writeList(self.__ADS1015_REG_POINTER_CONFIG, config_bytes)
    ...
    def startDifferentialComparator(self, chP, chN, thresholdHigh, thresholdLow, 
        ...
        # Write config register to the ADC.
        # Once written, the ADC will convert continuously and alert when conditions are met;
        # the converted values can then be read using getLastConversionResults.
        config_bytes = [(config >> 8) & 0xFF, config & 0xFF]
        self.i2c.writeList(self.__ADS1015_REG_POINTER_CONFIG, config_bytes)
